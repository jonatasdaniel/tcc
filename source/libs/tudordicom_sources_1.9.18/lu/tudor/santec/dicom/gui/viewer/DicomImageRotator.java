 ...
 * 
 */
public class DicomImageRotator {

	DicomImagePanel dicomImagePanel;

	public DicomImageRotator(DicomImagePanel dip) {

		this.dicomImagePanel = dip;

		// rotate image
		this.dicomImagePanel.addModeButton(
				DicomIcons.getScreenDependentIcon(DicomIcons.ACTION_ROTATE), Translatrix.getTranslationString("dicom.dicomImagePanel.rotateImage"),
				new ImageListener() {
					public void pixelClicked(ImagePlus ip, int x, int y) {
						ip.setProcessor("rotated", ip.getProcessor().rotateRight());
