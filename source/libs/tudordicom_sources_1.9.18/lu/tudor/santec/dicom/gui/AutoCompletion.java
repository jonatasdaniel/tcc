...
public class AutoCompletion extends PlainDocument {
	...
	public AutoCompletion(final JComboBox comboBox, boolean allowFreeText) {
...
//					break;
	...
	public static void enable(JComboBox comboBox) {
		...
		new AutoCompletion(comboBox, false);
	}

	public static void enableWithFreeText(JComboBox comboBox) {
		// has to be editable
		comboBox.setEditable(true);
		// change the editor's document
		new AutoCompletion(comboBox, true);
	}
...
//			System.out.println("Autocompletion: no Matching Item found");
		}
	}
...
//		editor.moveCaretPosition(start);
		...
		editor.moveCaretPosition(getLength());
	}
	...
	private static void createAndShowGUI() {
		...
		enableWithFreeText(comboBox);
		final JComboBox comboBox2 = new JComboBox(new Object[] { "Ester",
				"Jordi", "Jordina", "Jorge", "Sergi" });
		enable(comboBox2);
		// create and show a window containing the combo box
		final JFrame frame = new JFrame("AutoCompletion Example");
		frame.setLayout(new BorderLayout());
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().add(comboBox, BorderLayout.NORTH);
		frame.getContentPane().add(comboBox2, BorderLayout.CENTER);
		frame.getContentPane().add(new JTextArea(4,15), BorderLayout.SOUTH);
		frame.pack();
