...
public class DicomDirReader {
	...
	public boolean loadDicomDirFile(File dirFile, boolean create, boolean delete) {
		...
		try {
			...
			if (create) {
				...
				try {
					...
					System.out.println("Loading DICOMDIR at: " + STORE_DIR.getParentFile() +  "  ----------------------------");
				    } else {
					// create dicomdir
					dicomDir = new DicomDirWriter(STORE_DIR, fsinfo());
					System.out.println("Creating DICOMDIR at: " + STORE_DIR.getParentFile() +  "  ----------------------------");
				    }
			...
			} else if (delete && STORE_DIR.exists()) {
				// open dicomdir
		        dicomDir = new DicomDirWriter(STORE_DIR);
		        System.out.println("Loading DICOMDIR at: " + STORE_DIR.getParentFile() +  "  ----------------------------");
			} else {
...
//				String s = element.getString(Tag.SOPInstanceUID);
	...
	public static void main(String args[]) throws Exception {
		...
		ddr.loadDicomDirFile(new File("C:/Users/Dimitri/Desktop/DICOMDIR"), false, false);//new String("DICOMSTORE/DICOMDIR")
...
//        	}
	...
	if (f.getName().toUpperCase().endsWith("DICOMDIR"))
        ...
        if (f.isDirectory()) {
            ...
            // if old file not allready has the right filepath,
            if (! f.getAbsolutePath().equals(outFile.getAbsolutePath())) {

        	System.out.println("moving " + f.getAbsolutePath()  + " to " + outFile.getAbsolutePath());
    ...
    public String createFileName(DicomObject dcmobj) {
	StringBuffer fileName = new StringBuffer();
	fileName.append(dcmobj.getString(DICOM_DIR_NAMING[0]));
	for (int i = 1; i < DICOM_DIR_NAMING.length; i++) {
	    fileName.append(File.separator);
	    fileName.append(dcmobj.getString(DICOM_DIR_NAMING[i]));
	}
	// delete NUL bytes
