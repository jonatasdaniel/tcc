...
public class PhotoCube {
	...
	public PhotoCube(Context context, Dicom dicom, List<Bitmap> bitmaps) {
		textureIDs = new int[dicom.getImages().size()];
		
		this.dicom = dicom;
		this.bitmaps = bitmaps;

		// Allocate vertex buffer. An float has 4 bytes
		ByteBuffer vbb = ByteBuffer.allocateDirect(12 * 4 * textureIDs.length);
		vbb.order(ByteOrder.nativeOrder());
		vertexBuffer = vbb.asFloatBuffer();

		// Read images. Find the aspect ratio and adjust the vertices
		// accordingly.
		for (int face = 0; face < textureIDs.length; face++) {
			Bitmap bitmap = bitmaps.get(face);
	...
	public void loadTexture(GL10 gl) {
		gl.glGenTextures(textureIDs.length, textureIDs, 0); // Generate texture-ID array for 6
											// IDs
