 ...
 * 
 * InputStream.skip() looped to check the actual number of bytes is read.
 * 
 * Big/little-endian options on element length.
 * 
 * Explicit check for each known VR to make mistaken identifications of explicit
 * VR's less likely.
 * 
 ...
 * 
 * Queries on some other unrecognized tags. Anyone want to claim them?
 * 
 * RAK changes 15/7/99
 * 
 * Bug fix on magic values for explicit VRs with 32-bit lengths.
 * 
 * Various bits of tidying up, including... 'location' incremented on read using
 * getByte() or getString(). simpler debug mode message generation (values no
 * longer reported).
 * 
 * Added z pixel aspect ratio support for multi-slice DICOM volumes. Michael
 * Abramoff, 31-10-2000
 * 
 * Added DICOM tags to the dictionary (now contains about 2700 tags).
 * implemented getDouble() for VR = FD (Floating Double) and getFloat() for VR =
 * FL (Floating Single). Extended case statement in getHeaderInfo to retrieve FD
 * and FL values. Johannes Hermen, Christian Moll, 25-04-2008
 */
...
public class DICOM extends ImagePlus {
	...
	public void run(String arg) {
		...
		try {
			...
			if (msg.indexOf("EOF") < 0 && showErrors) {
				// System.out.println("DicomDecoder " + e.getClass().getName()
				// + "\n \n" + msg); TODO
				return;
				...
				System.out.println("ERROR - DicomDecoder - " + msg);
				return;
			}
		}
		if (gettingInfo) {
		...
		if (fi != null && fi.width > 0 && fi.height > 0 && fi.offset > 0) {
			...
			if (Prefs.openDicomsAsFloat) {
				...
				if (dd.rescaleIntercept != 0.0 && dd.rescaleSlope == 1.0)
					...
					&& (dd.rescaleSlope == 1.0 || fi.fileType == FileInfo.GRAY8)) {
			...
			if (dd.windowWidth > 0.0) {
				...
				// System.out.println("LOG - window: " + min + "-" + max); TODO
			}
			imageNumber = fi.imageNumber;
			...
			System.out.println("ERROR - DicomDecoder"
					+ " Unable to decode DICOM header.");
		dc = dd;
		// System.out.println("Status"); TODO
	}

	public HashMap<Integer, String> getInformations() {
		if (dc != null) {
			return dc.informations;
		} else {
			return null;
		}
	}
	...
	private String getDir(String file) {
		String strs[] = file.replace('\\', '/').split("/");
	...
	void convertToUnsigned(ImagePlus imp, FileInfo fi) {
		...
		// System.out.println("LOG - min: " + (min - 32768)); TODO
		if (min >= 32768) {
			for (int i = 0; i < pixels.length; i++)
...
class DicomDecoder {
	...
	static StringBuilder infoLog = new StringBuilder();
	public HashMap<Integer, String> informations = new HashMap<Integer, String>();
	public HashMap<Integer, String> values = new HashMap<Integer, String>();

	public DicomDecoder(String directory, String fileName) {
		// try {
		// this.log = new FileWriter(new
		// File("C:/Users/marcelo.oliveira/Desktop/Log.txt")); //TODO
		// this.log = new FileWriter(new
		// File("C:/Users/MarceloSimara/Desktop/Log.txt"));
		...
		if (dictionary == null) {
			...
			if (f.exists())
				try {
					...
					dictionary.load(is);
					is.close();
					// System.out.println("LOG - DicomDecoder: using "
					// + dictionary.size() + " tag dictionary at " + path); TODO
				} catch (Exception e) {
					dictionary = null;
				}
		}
		if (dictionary == null) {
			...
			// System.out.println("LOG - DicomDecoder: " + path
			// + " not found; using " + dictionary.size()
			// + " tag built in dictionary"); TODO
		}
	}
	...
	int getNextTag() throws IOException {
		...
		// System.out.println("LOG - getNextTag: "+tag+" "+elementLength); TODO
		return tag;
	}
	...
	FileInfo getFileInfo() throws IOException {
		...
		if (inputStream != null) {
			// Use large buffer to allow URL stream to be reset after reading
			// header
			f = inputStream;
			f.mark(400000);
		} else
			f = new BufferedInputStream(new FileInputStream(directory
					+ fileName));
		// System.out.println("DicomDecoder: decoding " + fileName); TODO
		...
		if (!getString(4).equals(DICM)) {
			...
			if (inputStream != null)
				...
				f = new BufferedInputStream(new FileInputStream(directory
						+ fileName));
			location = 0;
			// System.out.println("LOG - " + DICM + " not found at offset "
			// + ID_OFFSET + "; reseting to offset 0"); TODO
		} else {
			dicmFound = true;
			// System.out.println("LOG " + DICM + " found at offset " +
			// ID_OFFSET); TODO
		}
		...
		while (decodingTags) {
			...
			case IMAGE_NUMBER:
				...
				// int number = 0; //TODO
				// try {
				// String value[] = getHeaderInfo(tag, null).split(":");
				// number = Integer.parseInt(value[1].trim());
				// fi.imageNumber = number;
				// } catch (Exception e) {
				// //Não veio um número
				// // addInfo(tag, null);
				// }
		...
		// System.out.println("LOG - width: " + fi.width); TODO
		// System.out.println("LOG - height: " + fi.height);
		// System.out.println("LOG - images: " + fi.nImages);
		// System.out.println("LOG - bits allocated: " + bitsAllocated);
		// System.out.println("LOG - offset: " + fi.offset); TODO

		System.out.println(infoLog);

		// this.log.write(infoLog.toString());
		// this.log.close();
		if (inputStream != null)
			f.reset();
		else
	...
	void addInfo(int tag, String value) throws IOException {
		...
		// (info!=null && info.trim().equalsIgnoreCase("Image Number: ".trim()))
		// || (info!=null &&
		// info.trim().equalsIgnoreCase("Series Number: ".trim()))
		// || (info!=null &&
		// info.trim().equalsIgnoreCase("Acquisition Number: ".trim()))
		// if((info!=null &&
		// info.trim().equalsIgnoreCase("Image Number: ".trim()))
		// || (info!=null &&
		// info.trim().equalsIgnoreCase("Series Number: ".trim()))
		// || (info!=null &&
		// info.trim().equalsIgnoreCase("Acquisition Number: ".trim()))) {
		...
		infoLog.append(info);
		infoLog.append("\n");
		informations.put(tag, info);
		if (tag == IMAGE_NUMBER) {
			int number = 0; // TODO
			String str[] = info.split(":");
			number = Integer.parseInt(str[1].trim());
		...
		if (tag == 2097171 || tag == 0x00200012 || tag == 0x00200011
				|| tag == 2097170) {
			System.out.println("LOG - (" + tag2hex(tag) + VR + " "
					+ elementLength + " bytes from "
					+ (location - elementLength) + ") " + info + "   TAG - "
					+ tag); // TODO
			// log.write("LOG - (" + tag2hex(tag) + VR + " " + elementLength +
			// " bytes from "
			// + (location - elementLength) + ") " + info+"   TAG - " + tag +
			// " \n");
			// str.append("LOG - (" + tag2hex(tag) + VR + " " + elementLength +
			// " bytes from "
			// + (location - elementLength) + ") " + info+"   TAG - " + tag +
			// " \n"); TODO
		}
	...
	String getHeaderInfo(int tag, String value) throws IOException {
		...
		// case UT:
		// throw new
		// IOException("ImageJ cannot read UT (unlimited text) DICOMs");
		case AE:
		case AS:
		case AT:
		case CS:
		...
		case SQ:
			...
			if (tag != ICON_IMAGE_SEQUENCE && !privateTag)
				break;
			// else fall through and skip icon image sequence or private
			// sequence
		default:
...
class DicomDictionary {
	...
	String[] dict = {
			...
			"00408302=DSEntrance Dose in mGy", "0040A010=CSRelationship Type",
			"0040A027=LOVerifying Organization",
			"0040A030=DTVerification DateTime",
			"0040A032=DTObservation DateTime", "0040A040=CSValue Type",
			...
			"0040A0B0=USReferenced Waveform Channels", "0040A120=DTDateTime",
			"0040A121=DADate", "0040A122=TMTime", "0040A123=PNPerson Name",
			"0040A124=UIUID", "0040A130=CSTemporal Range Type",
			"0040A132=ULReferenced Sample Positions",
			"0040A136=USReferenced Frame Numbers",
			"0040A138=DSReferenced Time Offsets",
			"0040A13A=DTReferenced Datetime", "0040A160=UTText Value",
			"0040A168=SQConcept Code Sequence",
			"0040A180=USAnnotation Group Number",
			"0040A195=SQModifier Code Sequence",
			"0040A300=SQMeasured Value Sequence", "0040A30A=DSNumeric Value",
			...
			"0040A730=SQContent Sequence", "0040B020=SQAnnotation Sequence",
			"0040DB00=CSTemplate Identifier", "0040DB06=DTTemplate Version",
			...
			"00540018=SHEnergy Window Name", "00540020=USDetector Vector",
			"00540021=USNumber of Detectors",
			"00540022=SQDetector Information Sequence",
			"00540030=USPhase Vector", "00540031=USNumber of Phases",
			"00540032=SQPhase Information Sequence",
			"00540033=USNumber of Frames in Phase", "00540036=ISPhase Delay",
			"00540038=ISPause Between Frames", "00540039=CSPhase Description",
			"00540050=USRotation Vector", "00540051=USNumber of Rotations",
			...
			"00540070=USTime Slot Vector", "00540071=USNumber of Time Slots",
			"00540072=SQTime Slot Information Sequence",
			"00540073=DSTime Slot Time", "00540080=USSlice Vector",
			"00540081=USNumber of Slices", "00540090=USAngular View Vector",
			"00540100=USTime Slice Vector", "00540101=USNumber of Time Slices",
			"00540200=DSStart Angle", "00540202=CSType of Detector Motion",
			...
			"00540308=USEnergy Window Number", "00540400=SHImage ID",
			...
			"00540500=CSSlice Progression Direction", "00541000=CSSeries Type",
			"00541001=CSUnits", "00541002=CSCounts Source",
			"00541004=CSReprojection Method",
			"00541100=CSRandoms Correction Method",
			"00541101=LOAttenuation Correction Method",
			"00541102=CSDecay Correction", "00541103=LOReconstruction Method",
			"00541104=LODetector Lines of Response Used",
			"00541105=LOScatter Correction Method",
			"00541200=DSAxial Acceptance", "00541201=ISAxial Mash",
			"00541202=ISTransverse Mash", "00541203=DSDetector Element Size",
			...
			"00541320=DSSlice Sensitivity Factor", "00541321=DSDecay Factor",
			"00541322=DSDose Calibration Factor",
			"00541323=DSScatter Fraction Factor",
			"00541324=DSDead Time Factor", "00541330=USImage Index",
			"00541400=CSCounts Included",
			"00541401=CSDead Time Correction Flag",

			"30020002=SHRT Image Label", "30020003=LORT Image Name",
			"30020004=STRT Image Description",
			"3002000A=CSReported Values Origin", "3002000C=CSRT Image Plane",
			...
			"30020022=DSRadiation Machine SAD", "30020026=DSRT Image SID",
			"30020029=ISFraction Number", "30020030=SQExposure Sequence",
			"30020032=DSMeterset Exposure",

			"300A011E=DSGantry Angle", "300A0120=DSBeam Limiting Device Angle",
			...
			"300A00B3=CSPrimary Dosimeter Unit", "300A00F0=ISNumber of Blocks",
